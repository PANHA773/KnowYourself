...
export default function AdminMajors() {
  ...
  function load() {
    return http
      .get("/admin/majors")
      .then(({ data }) => setItems(data.majors || []))
      .catch((e) => setError(e?.response?.data?.message || i18n.t("admin.loadFail.majors")));
  }
  ...
  function select(item) {
    ...
    setForm({
      ...
      careers: toCsv(item.careers),
      careersKm: toCsv(item.careersKm)
    });
    setImageFile(null);
  }
  ...
  async function uploadMajorImage() {
    if (!imageFile) return;
    setError("");
    try {
      setUploading(true);
      ...
      const res = await http.post("/admin/uploads/major-image", data, {
  ...
  async function save() {
    ...
    try {
      ...
      if (isEditing) {
        await http.put(`/admin/majors/${active._id}`, form);
      } else {
        await http.post("/admin/majors", form);
      }
      await load();
      newItem();
    } catch (e) {
  ...
  async function remove() {
    ...
    if (!confirm(i18n.t("admin.confirmDeleteMajor"))) return;
    setError("");
    try {
      setSaving(true);
      await http.delete(`/admin/majors/${active._id}`);
      await load();
      newItem();
    } catch (e) {
    ...
    <div className="grid gap-4 md:grid-cols-[320px,1fr]">
      <Card>
        <div className="flex items-center justify-between">
          <div className="text-lg font-semibold">{i18n.t("admin.majors")}</div>
          ...
          {items.map((m) => (
            <button
              ...
              className={`text-left rounded-xl border border-white/10 px-3 py-2 transition-colors hover:bg-white/10 ${
                active?._id === m._id ? "bg-white/10" : "bg-white/5"
              }`}
            >
              <div className="text-sm font-semibold text-slate-100">{m.title}</div>
              {m.titleKm ? <div className="text-xs text-slate-200/70">{m.titleKm}</div> : null}
            </button>
          ))}
        </div>
      </Card>
      ...
      <Card>
        <div className="text-lg font-semibold">
          {isEditing ? i18n.t("admin.editMajor") : i18n.t("admin.createMajor")}
        </div>
        {error ? (
        ...
        <div className="mt-4 grid gap-3">
          ...
          <Field label={i18n.t("admin.field.imageUrl")} hint="/images/majors/graphic-design.svg">
            <input
              className="input"
              value={form.imageUrl}
              onChange={(e) => setForm({ ...form, imageUrl: e.target.value })}
              placeholder="/images/majors/information-technology.svg"
            />
          </Field>
          ...
          <Field label={i18n.t("admin.field.skillsEn")}>
            <input
              ...
              placeholder={i18n.t("admin.ph.csvSkills")}
            />
          </Field>
          ...
          <Field label={i18n.t("admin.field.careersEn")}>
            <input
              className="input"
              value={form.careers}
              onChange={(e) => setForm({ ...form, careers: e.target.value })}
              placeholder={i18n.t("admin.ph.csvCareers")}
            />
          </Field>
          <Field label={i18n.t("admin.field.careersKm")}>
            <input
              className="input"
              value={form.careersKm}
              onChange={(e) => setForm({ ...form, careersKm: e.target.value })}
            />
          </Field>
        </div>
